"""Tests for the SentinelNet Phase 8 anomaly-detection pipeline."""
...
def _build_phase8_fixture(rows_per_block: int = 24) -> pd.DataFrame:
...
def test_run_anomaly_detection_pipeline_trains_phase8_detectors() -> None:
    project_root = Path(__file__).resolve().parents[1] / ".tmp_tests" / "anomaly_detection_test_workspace"
    ...
    try:
        report = run_anomaly_detection_pipeline(config)
        ...
        assert (config.isolation_forest_dir / "isolation_forest.joblib").exists()
        assert (config.one_class_svm_dir / "one_class_svm.joblib").exists()
        assert (config.lof_dir / "lof.joblib").exists()
        assert (config.autoencoder_dir / "autoencoder.keras").exists()
        assert set(metrics_summary["model"]) == {"isolation_forest", "one_class_svm", "lof", "autoencoder"}
        assert set(metrics_summary["status"]) == {"trained"}
    finally:
        shutil.rmtree(project_root, ignore_errors=True)
