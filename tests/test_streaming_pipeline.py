"""Tests for the SentinelNet Phase 11 streaming pipeline."""
...
from src.deployment.streaming import run_streaming_pipeline
...
def _build_phase11_fixture(rows_per_block: int = 20) -> pd.DataFrame:
...
def test_run_streaming_pipeline_generates_real_time_predictions() -> None:
    project_root = Path(__file__).resolve().parents[1] / ".tmp_tests" / "streaming_test_workspace"
    ...
    streaming_config = StreamingConfig(
        ...
        simulation_start_utc="2026-01-01T00:00:00+00:00",
    )
    ...
    try:
        ...
        report = run_streaming_pipeline(streaming_config)
        predictions = pd.read_csv(streaming_config.predictions_path)

        assert report.validation_passed is True
        assert report.rows_streamed == 16
        assert predictions.shape[0] == 16
        assert report.selected_binary_variant in {"soft_voting", "weighted_scoring", "stacking"}
        assert report.selected_multiclass_variant in {"soft_voting", "weighted_scoring", "stacking"}
        assert "binary_attack_probability" in predictions.columns
        assert "multiclass_confidence" in predictions.columns
        assert predictions["stream_order"].iloc[0] == 0
        assert predictions["stream_order"].iloc[-1] == 15
    finally:
        shutil.rmtree(project_root, ignore_errors=True)
