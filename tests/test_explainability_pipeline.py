"""Tests for the SentinelNet Phase 10 explainability pipeline."""
...
from src.explainability.pipeline import run_explainability_pipeline
...
def _build_phase10_fixture(rows_per_block: int = 20) -> pd.DataFrame:
...
def test_run_explainability_pipeline_generates_phase10_artifacts() -> None:
    project_root = Path(__file__).resolve().parents[1] / ".tmp_tests" / "explainability_test_workspace"
    ...
    explainability_config = ExplainabilityConfig(
        ...
        phase7_output_dir=dl_config.output_dir,
        phase8_output_dir=anomaly_config.output_dir,
        phase9_output_dir=ensemble_config.output_dir,
        output_dir=project_root / "models" / "saved_models" / "phase10_explainability",
        logs_dir=project_root / "logs",
        background_sample_cap=8,
        binary_explain_cap=8,
        multiclass_explain_cap=8,
        ensemble_binary_explain_cap=8,
        ensemble_multiclass_explain_cap=8,
        shap_permutations=2,
    ...
    try:
        ...
        run_ensemble_pipeline(ensemble_config)
        report = run_explainability_pipeline(explainability_config)
        summary = pd.read_csv(explainability_config.summary_path)

        assert report.validation_passed is True
        assert len(report.artifacts) == 24
        assert len(summary) == 24
        assert report.selected_phase6_models["binary"]["task"] == "binary"
        assert report.selected_phase6_models["multiclass"]["task"] == "multiclass"
        assert (explainability_config.phase6_native_dir / "binary_lightgbm.csv").exists()
        assert (explainability_config.phase6_shap_dir / f"binary_{report.selected_phase6_models['binary']['model_name']}_summary.csv").exists()
        assert (explainability_config.phase9_dir / "binary_stacking_summary.csv").exists()
        assert (explainability_config.phase9_dir / "multiclass_weighted_scoring_local.csv").exists()
    finally:
        shutil.rmtree(project_root, ignore_errors=True)
